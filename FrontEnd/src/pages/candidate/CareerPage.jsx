...
import { Typography, Row, Col, Spin, Divider } from 'antd';
...
const CareerPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const positionsReq = positionApi.filter({ isActive: true, size: 50 });
        const applicationsReq = user ? candidateApi.getMyApplications() : Promise.resolve(null);

        const [posRes, appRes] = await Promise.all([positionsReq, applicationsReq]);

        setPositions(posRes.data?.content || []);

        if (appRes) {
          const map = {};
          (appRes.data || []).forEach((app) => {
            map[app.positionId] = app;
          });
            ...
            {positions.map((pos) => (
              <Col xs={24} sm={12} lg={8} key={pos.id} style={{ display: 'flex' }}>
                <div style={{ width: '100%' }}>
                  <PositionCard
                    position={pos}
                    applicationData={applicationMap[pos.id] || null}
                  />
                </div>
              </Col>
            ))}
