...
Graph node:
  - query_expansion_node: wraps expand_query() for LangGraph — only runs for
    strategies {RANK, FILTER, FIND_MORE}. Other strategies bypass this node.
"""
...
# ---------------------------------------------------------------------------
# LangGraph node — Step 8
# ---------------------------------------------------------------------------

# Strategies that benefit from query expansion (Qdrant retrieval is involved)
...
async def query_expansion_node(state: HRChatState) -> HRChatState:
    ...
    Skips expansion for all other strategies (COMPARE, DETAIL, ACTION, AGGREGATE)
    because those pipelines either bypass Qdrant entirely or use pinned fetch
    where expansion adds no value.

    Writes to state:
      - expanded_query:  enriched query string for dense embedding
    ...
    if strategy not in _EXPANSION_STRATEGIES:
        # Passthrough — write fallback values so downstream nodes always have the fields
        state["expanded_query"] = query
        state["skill_variants"] = skill_keywords
        print(f"[Expansion] strategy={strategy} → skipped (not in expansion strategies)")
        return state

    print(f"[Expansion] strategy={strategy} | keywords={skill_keywords} → calling LLM")
